...
```{r}
library(ggplot2)
da <- read.csv(unz("activity.zip","activity.csv"),colClasses=c("integer","Date","integer"))
```
Examine data.
...
```{r}
steps_per_interval <- aggregate(da$steps, by=list(interva=da$interval),FUN=mean, na.rm=T)
...
complete_da <- data.frame(  
    steps = impute_means(da, steps_per_interval),  
    date = da$date,  
...
calc_steps_per_interval <- function(tbl) {
    steps_pi <- aggregate(tbl$steps, by=list(interval=tbl$interval),FUN=mean, na.rm=T)
